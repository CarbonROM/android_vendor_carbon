...
func init() {
	android.RegisterModuleType("carbon_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = carbonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = carbonExpandVariables(ctx, dep_file)
	...
	cmd := carbonExpandVariables(ctx, String(g.properties.Cmd))
