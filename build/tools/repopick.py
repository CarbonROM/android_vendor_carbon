...
if __name__ == '__main__':
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit carbon', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch carbon', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or carbon failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
